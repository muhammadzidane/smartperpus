<div class="books {{ $class_books ?? '' }}">
    @foreach ($books as $book)
        <div class="book" data-id="{{ $book->id }}">
            ...
            <div class="pt-2 px-2 d-flex flex-column justify-content-between">
                <div>
            ...
            <div class="mt-auto pb-2 px-2">
                @if ($book->ratings)
                    @if ($book->book_user()->where('payment_status', 'arrived')->count() != 0)
                        <div class="">
                            <small class="d-block text-grey">{{ $book->book_user()->where('payment_status', 'arrived')->count() }} Terjual</small>
                        </div>
                    @endif
